...
Wolf.Game = (function() {
    ...
    var rendering = false,
        ...
        cycleNum = 0,
        mouseEnabled = false,
        leapEnabled = true, // default to true for dev
        paused = false,
        intermissionAnim = 0,
        currentGame = null,
        levelMusic,
    ...
    function updatePlayerControls(player, tics) {
        ...
        if (Wolf.Input.checkKeys(controls.attack) || (mouseEnabled && Wolf.Input.leftMouseDown()) ||
                (leapEnabled && Wolf.Input.thumbContracting())) {
        ...
        if (mouseEnabled && Wolf.Input.isPointerLocked()) {
            ...
            player.angle -= (Wolf.Input.getHandMovement() * Wolf.LEAPTURNANGLESCALE * tics)>>0;

            // always strafe when using leap motion
            if (leftKey) {
            ...
            if (mouseEnabled && (mouseCoords = Wolf.Input.getMouseCoords())) {
                if (Math.abs(mouseCoords.x) > Wolf.MOUSEDEADBAND) {
                    player.angle -= (Wolf.TURNANGLESCALE * tics * (mouseCoords.x +
                                (mouseCoords.x < 0 ? 1 : -1) * Wolf.MOUSEDEADBAND))>>0;
                }
            }
        }
        ...
        if (Wolf.Input.checkKeys(controls.use) || (leapEnabled && Wolf.Input.handClenching())) {
