...
Wolf.Input = (function() {
    ...
    function init() {
        ...
        // initialise leap motion controller and add event listeners
        Wolf.LeapController.init();
        Wolf.LeapController.on(Wolf.LeapListeners.OnAiming, function(palmVelocityX, hand) {
            handMoveX += palmVelocityX;
        });
        Wolf.LeapController.on(Wolf.LeapListeners.OnThumbContracting, function(angles, hand) {
            thumbContractingAngles = angles;
        });
        Wolf.LeapController.on(Wolf.LeapListeners.OnClenching, function(strength, hand) {
            clenchStrength = strength;
        });
    }
    ...
    function getHandMovement() {
        var x = handMoveX;

        handMoveX = 0;
        return x / Wolf.MAX_XVEL;
    }

    function thumbContracting() {
        return ((thumbContractingAngles.bottom <= Wolf.MAX_THUMBANGLE && thumbContractingAngles.bottom > 0) ||
                (thumbContractingAngles.top <= Wolf.MAX_THUMBANGLE-0.25 && thumbContractingAngles.top > 0));
    }

    function handClenching() {
        return (clenchStrength >= Wolf.MIN_CLENCH);
    }
    ...
    return {
        ...
        getHandMovement: getHandMovement,
        thumbContracting: thumbContracting,
        handClenching: handClenching,
